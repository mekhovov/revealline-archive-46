...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (886, 423289121))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.80.1'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-46/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], '9649852a5555b34687cbe5c9556952c0346bc55c')
        self.assertEqual(release['sourceRevision'], 'b810521a53af7be145acb8dedce0a01a747339cf')
        self.assertEqual(release['sourceTree'], '237dd46da3c0b98261997c13bc3a8fb5faac6bd0')
        self.assertEqual(release['sourceQualification'], {'bytes': 18235, 'sha256': '3d65968075c1f72ced158fb9905928d436319cd035055f0e4edf0fdf21156711'})
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.80.1/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (883, 423287727))
